...
Object.assign(physicsData.topics, {
  momentum: {
    ...
    canvas: "simMomentum",
    theory: `
      <h2><span class="t-num">6.1</span>โมเมนตัม (Momentum)</h2><p class="lead">โมเมนตัมเป็น<b>ปริมาณเวกเตอร์</b> ทิศเดียวกับความเร็ว การบวกหรือลบโมเมนตัมจึงต้องทำแบบเวกเตอร์เสมอ</p><section class="eq-core"><div class="eq-label">สมการตั้งต้น <span>· เวกเตอร์</span></div><div class="eq-block">$$\\begin{aligned}\\overrightharpoon p &= m\\overrightharpoon v \\\\ \\Delta\\overrightharpoon p &= \\overrightharpoon p_2 - \\overrightharpoon p_1 \\\\ &= m\\overrightharpoon v - m\\overrightharpoon u\\end{aligned}$$</div><ol class="derive"><li><p>แยกองค์ประกอบตามแกน</p><div class="eq-block">$$\\begin{aligned}p_x &= mv\\cos\\theta \\\\ p_y &= mv\\sin\\theta\\end{aligned}$$</div></li><li><p>ขนาดของโมเมนตัม</p><div class="eq-block">$$|\\overrightharpoon p| = \\sqrt{p_x^2 + p_y^2}$$</div></li><li><p>1 มิติ: <b>กำหนดทิศบวกก่อน</b> เช่น ลูกบอลกระทบผนังแล้วสะท้อนกลับ</p><div class="eq-block">$$\\begin{aligned}\\Delta p &= m(-v) - m(u) \\\\ &= -m(u + v)\\end{aligned}$$</div></li></ol></section><h3 class="eq-sec">สมการหลักที่ใช้บ่อย</h3><div class="eq-list"><div class="eq-row vec"><div class="eq-name">กฎข้อสองรูปโมเมนตัม</div><div class="eq-block">$$\\sum\\overrightharpoon F = \\frac{\\Delta\\overrightharpoon p}{\\Delta t}$$</div></div><div class="eq-row"><div class="eq-name">เชื่อมกับพลังงานจลน์</div><div class="eq-block">$$E_k = \\frac{p^2}{2m}$$</div></div><div class="eq-row warn"><div class="eq-name">สะท้อนกลับแนวเดิม</div><div class="eq-block">$$|\\Delta p| = m(u + v)$$</div></div><div class="eq-row"><div class="eq-name">หน่วย</div><div class="eq-block">$$\\text{kg}\\cdot\\text{m/s} = \\text{N}\\cdot\\text{s}$$</div></div></div><aside class="tips"><div class="tips-title">จำให้ขึ้นใจ</div><ul><li>สะท้อนกลับ ขนาดของ $\\Delta p$ คือ $m(u+v)$ <b>ไม่ใช่</b> $m(u-v)$</li><li>2 มิติ: $\\Delta\\overrightharpoon p = \\overrightharpoon p_2 + (-\\overrightharpoon p_1)$ วาดหัวต่อหาง หรือคิดทีละแกน</li></ul></aside><div class="sim-title">🎮 ลองเล่น: แอร์ฮอกกี้เก็บดาว <span class="badge">เกม</span></div><canvas id="simMomentum" class="sim-canvas"></canvas>
    `,
    problems: [
      {
        id: "ch6-1-1",
  ...
  impulse: {
    ...
    canvas: "simImpulse",
    theory: `
      <h2><span class="t-num">6.2</span>แรงและการดล (Impulse)</h2><p class="lead">การดลคือผลของแรงที่กระทำในช่วงเวลาหนึ่ง เท่ากับการเปลี่ยนโมเมนตัม และมี<b>ทิศเดียวกับ $\\Delta\\overrightharpoon p$</b></p><section class="eq-core"><div class="eq-label">สมการตั้งต้น <span>· เวกเตอร์</span></div><div class="eq-block">$$\\begin{aligned}\\overrightharpoon I &= \\overrightharpoon F\\,\\Delta t \\\\ &= \\Delta\\overrightharpoon p \\\\ &= m\\overrightharpoon v - m\\overrightharpoon u\\end{aligned}$$</div><ol class="derive"><li><p>มาจากกฎข้อสอง แล้วคูณ $\\Delta t$ ทั้งสองข้าง</p><div class="eq-block">$$\\sum\\overrightharpoon F = \\frac{\\Delta\\overrightharpoon p}{\\Delta t}$$</div></li><li><p>ถ้าแรงไม่คงตัว</p><div class="eq-block">$$I = \\text{พื้นที่ใต้กราฟ } F\\text{–}t$$</div></li></ol></section><h3 class="eq-sec">สมการหลักที่ใช้บ่อย</h3><div class="eq-list"><div class="eq-row"><div class="eq-name">แรงดลเฉลี่ย</div><div class="eq-block">$$F = \\frac{\\Delta p}{\\Delta t}$$</div></div><div class="eq-row"><div class="eq-name">กระทบแล้วหยุด</div><div class="eq-block">$$F = \\frac{mu}{\\Delta t}$$</div></div><div class="eq-row warn"><div class="eq-name">กระทบแล้วสะท้อนกลับ</div><div class="eq-block">$$F = \\frac{m(u + v)}{\\Delta t}$$</div></div><div class="eq-row"><div class="eq-name">หน่วย</div><div class="eq-block">$$\\text{N}\\cdot\\text{s} = \\text{kg}\\cdot\\text{m/s}$$</div></div></div><aside class="tips"><div class="tips-title">จำให้ขึ้นใจ</div><ul><li>$\\Delta p$ เท่ากัน แต่ $\\Delta t$ <b>ยาวขึ้น</b> → แรงดลเฉลี่ย<b>น้อยลง</b></li><li>ตัวอย่าง: ถุงลมนิรภัย หมอน การย่อเข่าตอนลงพื้น</li></ul></aside><div class="sim-title">🎮 ลองเล่น: ปล่อยไข่ลงพื้นแบบต่าง ๆ <span class="badge">ไข่จะรอดไหม</span></div><canvas id="simImpulse" class="sim-canvas"></canvas>
    `,
    problems: [
      {
        id: "ch6-2-1",
  ...
  collision: {
    ...
    canvas: "simCollision",
    theory: `
      <h2><span class="t-num">6.3</span>การชน (Collisions)</h2><p class="lead">เมื่อแรงลัพธ์ภายนอกเป็นศูนย์ <b>โมเมนตัมรวมคงตัว</b> สมการนี้เป็นสมการเวกเตอร์ — ใน 2 มิติต้องแยกคิดทีละแกน</p><section class="eq-core"><div class="eq-label">สมการตั้งต้น <span>· เวกเตอร์</span></div><div class="eq-block">$$\\begin{aligned}\\textstyle\\sum\\overrightharpoon p_{\\text{ก่อน}} &= \\textstyle\\sum\\overrightharpoon p_{\\text{หลัง}} \\\\ m_1\\overrightharpoon u_1 + m_2\\overrightharpoon u_2 &= m_1\\overrightharpoon v_1 + m_2\\overrightharpoon v_2\\end{aligned}$$</div><ol class="derive"><li><p>แยกแกน ได้สองสมการ</p><div class="eq-block">$$\\begin{aligned}\\textstyle\\sum p_x &= \\text{คงตัว} \\\\ \\textstyle\\sum p_y &= \\text{คงตัว}\\end{aligned}$$</div></li><li><p>มวลเท่ากัน ชนแบบยืดหยุ่นกับลูกที่หยุดนิ่ง</p><div class="eq-block">$$\\begin{aligned}\\overrightharpoon u &= \\overrightharpoon v_1 + \\overrightharpoon v_2 \\\\ u^2 &= v_1^2 + v_2^2\\end{aligned}$$</div></li><li><p>ยกกำลังสองแบบดอทแล้วเทียบกัน → สองลูก<b>แยกกันตั้งฉาก</b></p><div class="eq-block">$$2\\,\\overrightharpoon v_1\\cdot\\overrightharpoon v_2 = 0$$</div></li></ol></section><h3 class="eq-sec">สมการหลักที่ใช้บ่อย</h3><div class="eq-list"><div class="eq-row"><div class="eq-name">อนุรักษ์โมเมนตัม 1 มิติ</div><div class="eq-block">$$m_1u_1 + m_2u_2 = m_1v_1 + m_2v_2$$</div></div><div class="eq-row"><div class="eq-name">ยืดหยุ่น: พลังงานจลน์คงตัว</div><div class="eq-block">$$\\begin{aligned}&\\tfrac12 m_1u_1^2 + \\tfrac12 m_2u_2^2 \\\\ &= \\tfrac12 m_1v_1^2 + \\tfrac12 m_2v_2^2\\end{aligned}$$</div></div><div class="eq-row"><div class="eq-name">ยืดหยุ่น 1 มิติ (สูตรลัด)</div><div class="eq-block">$$u_1 + v_1 = u_2 + v_2$$</div></div><div class="eq-row warn"><div class="eq-name">ชนแล้วติดกันไป</div><div class="eq-block">$$v = \\frac{m_1u_1 + m_2u_2}{m_1 + m_2}$$</div></div><div class="eq-row vec"><div class="eq-name">การระเบิด / ดีดตัว</div><div class="eq-block">$$0 = m_1\\overrightharpoon v_1 + m_2\\overrightharpoon v_2$$</div></div></div><aside class="tips"><div class="tips-title">จำให้ขึ้นใจ</div><ul><li><b>ยืดหยุ่น</b>: ทั้งโมเมนตัมและพลังงานจลน์คงตัว</li><li><b>ไม่ยืดหยุ่น</b>: โมเมนตัมคงตัว แต่พลังงานจลน์บางส่วนกลายเป็นความร้อนหรือเสียง</li></ul></aside><div class="sim-title">🎮 ลองเล่น: ห้องทดลองการชน <span class="badge">1 มิติ / 2 มิติ</span></div><canvas id="simCollision" class="sim-canvas"></canvas>
    `,
    problems: [
      {
        id: "ch6-3-1",

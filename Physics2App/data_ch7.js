...
Object.assign(physicsData.topics, {
  projectile: {
    ...
    theory: `
      <h2><span class="t-num">7.1</span>การเคลื่อนที่แบบโพรเจกไทล์</h2><p class="lead">วัตถุเคลื่อนที่ใน 2 มิติ ด้วยความเร่งคงตัว $\\overrightharpoon g$ ที่<b>ชี้ลงเสมอ</b> เขียนเป็นสมการเวกเตอร์ชุดเดียว แล้วแยกคิดทีละแกน</p><section class="eq-core"><div class="eq-label">สมการตั้งต้น <span>· เวกเตอร์</span></div><div class="eq-block">$$\\begin{aligned}\\overrightharpoon v &= \\overrightharpoon u + \\overrightharpoon g\\,t \\\\ \\overrightharpoon s &= \\overrightharpoon u\\,t + \\tfrac12\\overrightharpoon g\\,t^2 \\\\ \\overrightharpoon g &= -g\\,\\hat j\\end{aligned}$$</div><ol class="derive"><li><p>แกน $x$: ไม่มีความเร่ง ความเร็วคงตัว</p><div class="eq-block">$$\\begin{aligned}v_x &= u\\cos\\theta \\\\ s_x &= (u\\cos\\theta)\\,t\\end{aligned}$$</div></li><li><p>แกน $y$: ความเร่ง $-g$</p><div class="eq-block">$$\\begin{aligned}v_y &= u\\sin\\theta - gt \\\\ s_y &= (u\\sin\\theta)\\,t - \\tfrac12 gt^2 \\\\ v_y^2 &= u_y^2 - 2gs_y\\end{aligned}$$</div></li><li><p>รวมกลับเป็นความเร็วลัพธ์</p><div class="eq-block">$$\\begin{aligned}|\\overrightharpoon v| &= \\sqrt{v_x^2 + v_y^2} \\\\ \\tan\\alpha &= \\frac{v_y}{v_x}\\end{aligned}$$</div></li></ol></section><h3 class="eq-sec">สมการหลักที่ใช้บ่อย</h3><div class="eq-list"><div class="eq-row"><div class="eq-name">เวลาถึงจุดสูงสุด</div><div class="eq-block">$$t_{top} = \\frac{u\\sin\\theta}{g}$$</div></div><div class="eq-row"><div class="eq-name">เวลาลอยทั้งหมด (ตกระดับเดิม)</div><div class="eq-block">$$T = \\frac{2u\\sin\\theta}{g}$$</div></div><div class="eq-row"><div class="eq-name">ความสูงสูงสุด</div><div class="eq-block">$$H = \\frac{u^2\\sin^2\\theta}{2g}$$</div></div><div class="eq-row"><div class="eq-name">พิสัยแนวราบ (ไกลสุดที่ 45°)</div><div class="eq-block">$$R = \\frac{u^2\\sin 2\\theta}{g}$$</div></div><div class="eq-row warn"><div class="eq-name">ยิงแนวระดับจากที่สูง h</div><div class="eq-block">$$\\begin{aligned}t &= \\sqrt{\\frac{2h}{g}} \\\\ R &= ut\\end{aligned}$$</div></div><div class="eq-row"><div class="eq-name">สมการเส้นทาง</div><div class="eq-block">$$y = x\\tan\\theta - \\frac{gx^2}{2u^2\\cos^2\\theta}$$</div></div></div><aside class="tips"><div class="tips-title">จำให้ขึ้นใจ</div><ul><li>ดอท $\\overrightharpoon v\\cdot\\overrightharpoon g = -g\\,v_y$ : ขาขึ้นติดลบ อัตราเร็ว<b>ลดลง</b> · ขาลงเป็นบวก อัตราเร็ว<b>เพิ่มขึ้น</b></li><li>จุดสูงสุด $v_y = 0$ แต่ $\\overrightharpoon v = v_x\\hat i$ <b>ไม่เป็นศูนย์</b></li></ul></aside><div class="sim-title">🎮 ลองเล่น: ยิงลูกให้ลงตะกร้า <span class="badge">เกม</span></div><canvas id="simProjectile" class="sim-canvas"></canvas>
    `,
    problems: [
      {
        id: "proj_1",

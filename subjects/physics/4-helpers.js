/* ============================================================================
   ตัวช่วยเฉพาะวิชา — subjects/physics/4-helpers.js
...
function figForcesPoint(o) {
  ...
  const list = o.forces || [];
  /* ── ความยาวลูกศรตามสัดส่วนของขนาดจริง ───────────────────────────────
     ลูกศรตัวไหนส่ง mag (ขนาดจริง) มาด้วย จะถูกวาดยาวตามอัตราส่วนของขนาดนั้น
     รูปจึงไม่หลอกตา — องค์ประกอบที่มากกว่าต้องยาวกว่าให้เห็นชัด
     ตัวที่ไม่ได้ส่ง mag ยังใช้ len เหมือนเดิม รูปเก่าทุกรูปจึงไม่กระทบ */
  const mx = Math.max.apply(null, list.map(f => Number(f.mag) || 0).concat([0]));
  const maxLen = o.maxLen || 96;
  const lenOf = f => (f.mag && mx ? Math.max(26, maxLen * Number(f.mag) / mx) : (f.len || 62));
  const tip = f => {
    const a = -f.deg * Math.PI / 180, r = lenOf(f);
    return [cx + r * Math.cos(a), cy + r * Math.sin(a)];
  };
  let s = '';
  /* แกนอ้างอิงจาง ๆ ให้เห็นว่ามุมวัดจากแนวไหน */
  if (o.axes) {
    s += '<line class="dash" x1="' + (cx - 30) + '" y1="' + cy + '" x2="' + (cx + 128) + '" y2="' + cy + '"/>' +
         '<line class="dash" x1="' + cx + '" y1="' + (cy + 30) + '" x2="' + cx + '" y2="' + (cy - 116) + '"/>';
  }
  /* เส้นประจากปลายลูกศรไปตั้งฉากกับแกนทั้งสอง — ใช้ตอนสอนการแยกองค์ประกอบ
     เมื่อความยาวเป็นสัดส่วนเดียวกันทั้งรูป มุมของสี่เหลี่ยมนี้จะไปบรรจบกับ
     ปลายลูกศรองค์ประกอบพอดี นักเรียนจึงเห็นว่าองค์ประกอบมาจากไหน */
  (o.drop || []).forEach(d => {
    const t = tip(d);
    s += '<line class="dash" x1="' + n1(t[0]) + '" y1="' + n1(t[1]) + '" x2="' + n1(t[0]) + '" y2="' + cy + '"/>' +
         '<line class="dash" x1="' + n1(t[0]) + '" y1="' + n1(t[1]) + '" x2="' + cx + '" y2="' + n1(t[1]) + '"/>';
  });
  list.forEach(f => {
    const t = tip(f), a = -f.deg * Math.PI / 180;
    s += svgArrow(cx, cy, t[0], t[1], 'frc', f.label,
      (f.lx == null ? Math.cos(a) * 18 : f.lx),
      (f.ly == null ? Math.sin(a) * 18 + 5 : f.ly));
  });
  /* ส่วนโค้งบอกมุม วัดทวนเข็มนาฬิกาจากแกน x (หรือจาก from ถึง to) */
  (o.arcs || []).forEach(g => {
    const r = g.r || 36, d0 = g.from || 0, d1 = g.to;
    const p = d => [cx + r * Math.cos(-d * Math.PI / 180), cy + r * Math.sin(-d * Math.PI / 180)];
    const A = p(d0), B = p(d1);
    s += '<path class="ln2" fill="none" d="M ' + n1(A[0]) + ' ' + n1(A[1]) +
         ' A ' + r + ' ' + r + ' 0 0 ' + (d1 > d0 ? 0 : 1) + ' ' + n1(B[0]) + ' ' + n1(B[1]) + '"/>';
    const mid = (d0 + d1) / 2 * Math.PI / 180;
    s += svgTxt(cx + (r + 17) * Math.cos(mid), cy - (r + 17) * Math.sin(mid) + 5, g.label, 'middle');
  });

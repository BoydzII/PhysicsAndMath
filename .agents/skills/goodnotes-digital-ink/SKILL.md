...
- **สถาปัตยกรรม Layer แยกเด็ดขาด (`#presentationCanvas`):**
  1. สร้างเลเยอร์ `<canvas id="presentationCanvas">` ซ้อนทับด้านบน (`z-index: 25; pointer-events: none;`)
  2. **Laser Pointer (`laser`):**
     - เรนเดอร์ลำแสงสีแดงสดใส พร้อมหางจางลงตามกาลเวลา (`fading trail ~1.0s`)
     - มีจุดศูนย์กลางสีขาวสว่างจ้า (Hot Core) และประกายเรืองแสงสีแดง (Outer Red Glow)
     - ปรับขนาดรัศมีตามปุ่มขนาด (`S` = 5px, `M` = 9px, `L` = 16px)
  3. **Pointing Finger (`finger`):**
     - เรนเดอร์มือชี้ (👉) พร้อมวงแหวนพัลส์สีน้ำเงินเรืองแสงล้อมรอบจุดโฟกัส
     - เหมาะสำหรับการชี้สูตรฟิสิกส์ ตัวแปร หรือแผนภาพให้นักเรียนดูได้อย่างชัดเจน
     - ปรับขนาดตามปุ่มขนาด (`S` = 28px, `M` = 40px, `L` = 58px)
  4. ไม่สร้าง Undo/Redo State และไม่รบกวนข้อมูลลายมือถาวรใน LocalStorage

### ช) การจัดลำดับชั้นให้ปากกาและไฮไลต์อยู่บนสุดทับข้อความโจทย์ (Highlight Over Text Blending)
- **โจทย์ของผู้ใช้:** ต้องการเลือกให้ปากกาและไฮไลต์อยู่บนสุด เพื่อให้สามารถเน้นข้อความ ไฮไลต์คำสำคัญ หรือเขียนกำกับทับตัวหนังสือโจทย์ได้โดยตรง
- **สถาปัตยกรรม:**
  1. กำหนดคลาส `.notebook-wrapper.canvas-on-top #drawingCanvas`:
     ```css
     .notebook-wrapper.canvas-on-top #drawingCanvas {
       z-index: 5;
       mix-blend-mode: multiply;
     }
     ```
  2. ด้วย CSS `mix-blend-mode: multiply`:
     - ปากกาไฮไลต์สีเหลืองจะผสมสีกับพื้นหลังสีขาวกลายเป็นสีเหลืองสดใส
     - เมื่อลากทับตัวหนังสือสีดำ หมึกสีดำจะยังคงคมชัด 100% ไม่ถูกบังหรือกลืนหายไป
  3. มีปุ่ม `#btnHighlightLayer` บนแถบเครื่องมือ ให้ครูและนักเรียนสามารถสลับโหมดการวางเลเยอร์ (ทับโจทย์ vs อยู่ใต้ตัวหนังสือ) ได้ทันที
...
- [ ] โหมด Pen-Only (Palm Rejection) บล็อกนิ้วมือจากการวาดเส้น 100% ขณะที่อนุญาตให้เลื่อนและซูมได้อย่างอิสระ
- [ ] เลเซอร์พอยน์เตอร์และนิ้วชี้ ปรับขนาด S, M, L ได้ เรนเดอร์ลื่นไหลบน `#presentationCanvas` และไม่ทำให้ Undo stack สกปรก
- [ ] ปากกาเน้นข้อความสามารถเขียนทับข้อความโจทย์ได้คมชัดด้วย `mix-blend-mode: multiply`
